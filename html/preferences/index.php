...
if (isset($_REQUEST['newtimezone'])) {
  $newtimezone = $_REQUEST['newtimezone'];
} elseif (isset($_GET['newtimezone'])) {
  $newtimezone = $_GET['newtimezone'];
} else {
  $newtimezone = '';
}
...
// User Actions
if ($action == "settimezone" && $newtimezone <> "") {
  $timezone = $newtimezone;
  $_SESSION["timezone"] = $timezone;
  $dbconn->query("UPDATE users SET timezone = '" . $newtimezone . "' WHERE pkid = '" . $userpkid . "'");
}
			...
			<div class="container">
				<div class="row">
					<section class="col-12">
						<p>You are logged in as <?php echo ($displayname); ?> (<?php echo ($email); ?>).</p>
						<br>
						<form method="post">
							<input type="hidden" name="action" value="settimezone" label="Time Zone:">
							<label for="newtimezone">Set Timezone:</label>
							<select name="newtimezone" onchange="this.form.submit()">
						...
						if ($isadmin) {
							...
							echo ("  Enable Debug: \n");
							echo ("  <label class=\"checkboxcontainer\">\n");
